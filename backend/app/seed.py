...
def seed_data():
    ...
    try:
        ...
        if not db.query(User).filter(User.username == "admin").first():
            db.add(User(username="admin", full_name="Ashan Perera", password_hash=hash_password("admin123"), role="admin"))
        if not db.query(User).filter(User.username == "nimal").first():
            db.add(User(username="nimal", full_name="Nimal K.", password_hash=hash_password("nimal123"), role="employee"))
        if not db.query(User).filter(User.username == "dimuth").first():
            db.add(User(username="dimuth", full_name="Dimuth R.", password_hash=hash_password("dimuth123"), role="employee"))
        if not db.query(User).filter(User.username == "kusal").first():
            db.add(User(username="kusal", full_name="Kusal P.", password_hash=hash_password("kusal123"), role="employee"))
        db.flush()
