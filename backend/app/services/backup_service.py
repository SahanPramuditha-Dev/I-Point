...
def create_backup(db: Session, is_auto: bool = False, trigger: str = "manual") -> dict[str, Any]:
    ...
    if settings.backup_encrypt:
        ...
        if passphrase:
            try:
                ...
                encryption_reason = str(exc)
                logger.warning(f"Backup encryption skipped due to error: {exc}")
        else:
            encryption_reason = "missing passphrase"
            logger.warning("Backup encryption enabled but passphrase is missing; storing compressed backup only.")
    ...
    if settings.firebase_backup_enabled and settings.firebase_prune_remote_keep > 0:
        try:
            prune_remote_backups(prefix="istore-backups/", keep=settings.firebase_prune_remote_keep)
        except Exception as exc:
