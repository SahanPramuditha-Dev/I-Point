...
def _next_code(db: Session, model, prefix: str) -> str:
    return f"{prefix}-{int(datetime.utcnow().timestamp())}-{uuid.uuid4().hex[:6].upper()}"
...
@router.post("/cash-reconciliation")
def create_cash_reconciliation(
    payload: CashReconciliationIn,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    recon_date = _parse_iso_date(payload.recon_date) or datetime.utcnow()
...
@router.put("/cash-reconciliation/{recon_id}/resolve")
...
@router.post("/daily-closing/generate")
def generate_daily_closing(
    payload: FinancialDailyClosingGenerateIn,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    base_date = _parse_iso_date(payload.report_date) or datetime.utcnow()
...
@router.put("/daily-closing/{closing_id}/verify")
def verify_daily_closing(
    ...
    row.status = "Signed"
    row.verified_by_user_id = current_user.id if current_user else None
    row.verification_time = datetime.utcnow()
    if payload.notes:
...
@router.put("/transactions/{transaction_type}/{transaction_id}/review")
    ...
    if payload.status == "Flagged":
        db.add(
            FinancialAuditFlag(
                ...
                transaction_type=normalized_type,
                transaction_id=transaction_id,
                reference_code=f"manual-txn-{normalized_type}-{transaction_id}-{int(datetime.utcnow().timestamp())}",
            )
        )
    db.commit()
    db.refresh(review)
...
def create_financial_flag(
    ...
    row = FinancialAuditFlag(
        ...
        reference_code=payload.reference_code or f"user-{int(datetime.utcnow().timestamp())}",
...
@router.put("/flags/{flag_id}/escalate")
def escalate_financial_flag(
    ...
    if row.resolution_notes:
        ...
        row.resolution_notes = f"Escalated at {datetime.utcnow().isoformat()}."
    db.commit()
    db.refresh(row)
    return _flag_record(row)


@router.put("/flags/bulk-resolve")
