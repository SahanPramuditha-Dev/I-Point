...
from app.schemas import PurchaseOrderIn, PurchaseReconcileIn

router = APIRouter(prefix="/purchase", tags=["purchase"])


def _next_po_number(db: Session) -> str:
    return f"PO-{int(db.query(PurchaseOrder).count()) + 10001}"


def _next_grn_number() -> str:
    return f"GRN-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
...
def _serialize_po(po: PurchaseOrder, include_items: bool = False, include_grns: bool = False) -> dict:
    ...
    result = {
        ...
        "grn_count": len(grns),
        "grn_numbers": [g.grn_no for g in grns],
    }
    if include_items:
        result["items"] = [
            {
...
def _reconcile_po(
    ...
    *,
    ...
    existing_grn = (
        ...
        .filter(GoodsReceivedNote.po_id == po.id)
...
def list_reconciliation(db: Session = Depends(get_db), _=Depends(get_current_user)):
    rows = (
        db.query(PurchaseOrder)
        .options(
            joinedload(PurchaseOrder.supplier),
        ...
        out.append(
            {
                ...
                "grn_count": len(po.grns or []),
                "grn_numbers": [row.grn_no for row in (po.grns or [])],
            }
        )
    return out
