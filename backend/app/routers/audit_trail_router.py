...
def audit_trail_events(
    ...
    sales_lookup_rows = db.query(Sale.id, Sale.customer_id).all()
    invoice_map = {int(row[0]): _format_invoice_id(int(row[0])) for row in sales_lookup_rows}

    repair_lookup_rows = db.query(RepairTicket.id, RepairTicket.ticket_no).all()
    repair_map = {int(row[0]): (row[1] or f"REP-{int(row[0]):05d}") for row in repair_lookup_rows}

    inventory_lookup_rows = db.query(InventoryItem.id, InventoryItem.sku, InventoryItem.name).all()
    inventory_map = {
        int(row[0]): {
            "sku": str(row[1] or ""),
    ...
    restore_setting = db.query(AppSetting).filter(AppSetting.key == "backup_restore_requests_v1").first()
    restore_rows = []
    if restore_setting and restore_setting.value:
        try:
            parsed = json.loads(restore_setting.value)
            if isinstance(parsed, list):
                restore_rows = [x for x in parsed if isinstance(x, dict)]
        except Exception:
            restore_rows = []

    for idx, row in enumerate(restore_rows):
        status_value = str(row.get("status") or "Pending Approval")
        if status_value not in {"Executed", "Failed", "Rejected", "Approved", "Pending Approval"}:
            continue
        ts = row.get("executed_at") or row.get("requested_at") or row.get("approved_at") or row.get("rejected_at")
        ts_dt = _coerce_dt(ts) if ts else None
        if start_dt and ts_dt and ts_dt < start_dt:
            continue
        if end_dt and ts_dt and ts_dt >= end_dt:
            continue
        action_label = "Restore Performed" if status_value in {"Executed", "Failed"} else "Create"
        module_label = "Backup & Restore"
        if status_value in {"Failed", "Rejected"}:
            status_label = "Failed"
        elif status_value in {"Pending Approval", "Approved"}:
            status_label = "Warning"
        else:
            status_label = "Success"
        detail = row.get("execution_result") or row.get("reason") or f"Restore request {status_value.lower()}"
        severity = _infer_severity(action_label, module_label, status_label, detail)
        sensitive = _sensitive_reason(action_label, module_label, status_label, detail)
        actor_label = row.get("executed_by") or row.get("approved_by") or row.get("requested_by") or "System"
        events.append(
            {
                "event_id": f"restore_request:{row.get('request_id') or idx}",
                "actor_user_id": None,
                ...
                "action": action_label,
                "module": module_label,
                "target_record": row.get("filename") or "Backup Restore Request",
                "old_value": None,
                "new_value": {
                ...
                "archived": f"restore_request:{row.get('request_id') or idx}" in archived_ids,
                "archived_info": archived_meta.get(f"restore_request:{row.get('request_id') or idx}"),
            }
        )
...
def archive_audit_events(
    ...
    archived_at = _to_iso(datetime.utcnow())
    actor = {
        "user_id": current.id,
