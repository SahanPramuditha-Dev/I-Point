...
def _last_print_map(
    ...
    *,
    ...
    rows = query.order_by(LabelPrintJob.completed_at.desc(), LabelPrintJob.created_at.desc()).all()
...
def labels_meta(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    technicians = sorted(list({(row.technician or "").strip() for row in db.query(RepairTicket).all() if (row.technician or "").strip()}))
...
def labels_dashboard(
    ...
    repair_rows = db.query(RepairTicket).all()
    ...
    new_arrivals_today = [
        row
        for row in inventory_items
        if row.id and row.id > 0 and row.name
    ]
...
def list_label_templates(
    ...
    ensure_label_defaults(db)
    query = db.query(LabelTemplate).options(joinedload(LabelTemplate.created_by))
...
@router.put("/queue/{job_id}/status")
...
@router.post("/queue/{job_id}/print-now")
...
def clear_completed_queue(
    ...
    rows = db.query(LabelPrintJob).filter(LabelPrintJob.status.in_(["Completed", "Cancelled"])).all()
    count = len(rows)
    for row in rows:
        db.delete(row)
    db.commit()
...
def retry_failed_queue(
    ...
    rows = db.query(LabelPrintJob).filter(LabelPrintJob.status == "Failed").all()
    for row in rows:
        row.status = "Waiting"
        row.error_message = None
    db.commit()
...
def label_history(
    ...
    return {
        "kpis": {
            ...
            "print_jobs_today": len([r for r in rows if r.created_at and r.created_at >= datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)]),
