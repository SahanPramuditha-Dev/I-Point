...
DEFAULT_SETTINGS_STATE = {
    ...
    "access_control": {
        "role_definitions": [
            ...
            {"role": "View Only", "level": 0, "description": "Read-only, no edits"},
        ],
        "permission_matrix": [
...
def _normalize_employee_profile(profile: dict | None) -> dict:
    ...
    if not merged.get("created_on"):
        merged["created_on"] = datetime.utcnow().isoformat()
    return merged
    ...
    legacy_print.update(
        {
            ...
            "store_name": business_identity.get("shop_name", legacy_print.get("store_name", "i Store")),
...
@router.put("/section/{section_key}")
...
@router.get("/access-control/rbac")
def access_control_rbac(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    role_definitions = []
    ordered = ["owner", "admin", "manager", "technician", "cashier", "view_only"]
    for name in ordered:
...
@router.put("/access-control/roles/{role_id}/permissions")
...
@router.post("/access-control/roles/{role_id}/grant-all")
...
@router.post("/access-control/roles/{role_id}/revoke-all")
...
@router.get("/access-control/users/{user_id}/overrides")
...
@router.put("/access-control/users/{user_id}/overrides")
...
@router.delete("/access-control/users/{user_id}/overrides/{permission_id}")
...
@router.get("/access-control/sessions")
...
@router.post("/access-control/sessions/{session_id}/terminate")
...
@router.post("/access-control/sessions/terminate-all")
...
@router.get("/access-control/users/{user_id}/effective-permissions")
...
@router.post("/access-control/users/{user_id}/unlock")
