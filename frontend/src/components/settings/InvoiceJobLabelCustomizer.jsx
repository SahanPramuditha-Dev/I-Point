...
function defaultSalesSettings(format = "a4") {
  return {
    ...
    footer: {
      show_thank_you: true,
      thank_you_text: "Thank you for shopping at i Store!",
      thank_you_size: 11,
      ...
      custom_line_3: "Follow us: @iStore.lk",
...
function defaultJobSettings(format = "a4") {
  return {
    ...
    footer: {
      ...
      footer_text: "Thank you for trusting iStore with your device.",
      show_tear_off: true,
...
function PreviewSalesBill({ settings, previewMode }) {
    ...
    <div className="rounded-xl border border-dashed border-slate-500/40 p-5" style={{ background: bg, color: textColor, fontFamily: settings?.print?.font_family || "DM Sans", fontSize: `${settings?.print?.base_font_size || 11}px` }}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {settings?.branding?.show_shop_name && <h3 className="font-black text-lg" style={{ color: settings?.branding?.shop_name_color || textColor }}>{settings?.branding?.shop_name_text || "i Store"}</h3>}
          {settings?.branding?.show_tagline && <p className="text-[11px]" style={{ color: settings?.branding?.tagline_color || "#777" }}>{settings?.branding?.tagline_text || "-"}</p>}
        </div>
        <div className="text-right text-[11px] space-y-0.5" style={{ color: settings?.business?.color || "#666" }}>
          {settings?.business?.show_address && <p>No 123, Main Street, Colombo</p>}
          {settings?.business?.show_phone && <p>077 123 4567</p>}
          {settings?.business?.show_email && <p>info@istore.lk</p>}
          {settings?.business?.show_website && <p>www.istore.lk</p>}
        </div>
      </div>
...
export default function InvoiceJobLabelCustomizer({
  ...
  const createTemplate = () => {
    const name = window.prompt("Template name", `${documentId === "sales_bill" ? "Sales" : documentId === "job_card" ? "Job" : "Label"} Template`);
    if (!name) return;
    const baseSettings =
  ...
  const renameTemplate = () => {
    if (!selectedTemplate) return;
    const name = window.prompt("Rename template", selectedTemplate.name || "Template");
    if (!name) return;
    const next = clone(customizer);
    const idx = next.templates.findIndex((row) => row.id === selectedTemplate.id);
    if (idx < 0) return;
              ...
              >
                ...
                <div className="rounded-2xl border border-white/10 bg-black/20 p-4 overflow-auto max-h-[780px] custom-scrollbar">
                  <div className="origin-top-left" style={{ transform: `scale(${scale})`, transformOrigin: "top left" }} ref={previewRef}>
                    {documentId === "sales_bill" && <PreviewSalesBill settings={settings} previewMode={customizer.ui.preview_mode} />}
