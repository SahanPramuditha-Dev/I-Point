      ...
      render: ({ data, updatePath }) => (
        <div className="space-y-3">
          ...
          <div className="flex justify-end">
            <Button size="sm" variant="secondary" onClick={() => toast("Full data export queued (simulation).", "info")}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              onClick={async () => {
                const ok = await confirm("Run Cleanup", "Run selected cleanup routines (simulation)?");
                if (!ok) return;
                toast("Cleanup flow validated (simulation).", "warning");
              }}
            >
              <Trash2 size={13} /> Run Cleanup
            </Button>
          </div>
        </div>
      ),
