...
export default function ReportsModuleLayout() {
  ...
  const { data: salesRaw, loading: salesLoading } = useFetch(`/reports/sales${queryRange}`);
  const { data: repairsRaw, loading: repairsLoading } = useFetch(`/reports/repairs${queryRange}`);
  const { data: summaryRaw, loading: summaryLoading } = useFetch(`/reports/summary${queryRange}`);
  const { data: inventoryRaw, loading: inventoryLoading } = useFetch("/reports/inventory");
  const { data: repairTicketsRaw, loading: repairTicketsLoading } = useFetch("/repairs");
  const { data: customersRaw, loading: customersLoading } = useFetch("/customers");
  const { data: suppliersRaw, loading: suppliersLoading } = useFetch("/inventory/suppliers");
  const { data: purchaseRaw, loading: purchaseLoading } = useFetch("/purchase");
  const { data: expensesRaw, loading: expensesLoading } = useFetch(`/reports/expenses${queryRange}`);
  const { data: movementsRaw, loading: movementsLoading } = useFetch("/inventory/movements");
  const { data: dashboardRaw, loading: dashboardLoading } = useFetch("/dashboard");
  const { data: notificationsRaw, loading: notificationsLoading } = useFetch("/notifications");
  const { data: auditActivityRaw, loading: auditActivityLoading } = useFetch(`/reports/audit-activity${queryRange}`);
  const { data: auditRepairHistoryRaw, loading: auditRepairHistoryLoading } = useFetch(`/reports/audit-repair-history${queryRange}`);
  const { data: priceAdjustmentsRaw, loading: priceAdjustmentsLoading } = useFetch("/inventory/price-adjustments");
  const { data: discountsRaw, loading: discountsLoading } = useFetch("/inventory/discounts");
  const { data: backupsRaw, loading: backupsLoading } = useFetch("/backup");
  const { data: employeesRaw, loading: employeesLoading } = useFetch("/settings/employees");
