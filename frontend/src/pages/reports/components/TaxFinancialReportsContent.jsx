...
import { Badge, KpiCard, SectionCard, Table } from "../../../components/UI";
...
export default function TaxFinancialReportsContent({
  ...
  const normalizedRepairs = useMemo(() => {
    return (repairRows || [])
      ...
      .map((row) => {
        const grossAmount = Math.max(0, Number(row.estimated_cost || 0));
        ...
        const taxAmount = vatTax + otherTax;
        const netAmount = grossAmount - taxAmount;
        const paid = Math.min(grossAmount, Math.max(0, Number(row.advance_payment || 0)));
    ...
    () =>
      dailyFinancialRows.map((row) => {
        ...
        return {
          ...
          sourceNote: "Auto-estimated (cash count endpoint not available).",
        };
      }),
    [dailyFinancialRows],
  );
    ...
    const payloads = {
      ...
      "vat-return": {
        exportColumns: [
          ...
          { label: "Input VAT (Est.)", value: (row) => Number(row.inputVatEstimate || 0) },
    ...
    if (activeSubReport === "vat-return") {
      return (
        <SectionCard
          ...
          subtitle="Ready-to-file summary. Input VAT is estimated as 0 until purchase tax fields are available."
        >
          <MiniTable
            columns={[
              { label: "Period", value: "period" },
              ...
              { label: "Input VAT (Est.)", value: (row) => money(row.inputVatEstimate) },
    ...
    if (activeSubReport === "cash-recon") {
      return (
        <SectionCard
          ...
          subtitle="Expected vs actual cash (actual uses system estimate until manual cash count logs are available)."
        >
          <MiniTable
            columns={[
              { label: "Date", value: "date" },
    ...
    <div className="space-y-3">
      ...
      <SectionCard title="Tax & Financial Tables">
        ...
        <p className="mt-2 text-xs text-slate-400">
          Repair service charge and cash reconciliation rows use best-effort estimates when exact accounting fields are not available in current endpoints.
        </p>
      </SectionCard>
