...
export default function RepairReportsContent({
  ...
  const analytics = useMemo(() => {
    ...
    const repairRevenue = nonCancelledRows.reduce((acc, row) => acc + Number(row.estimated_cost || 0), 0);
    ...
    const technicianRows = Object.values(
      filteredRows.reduce((acc, row) => {
        ...
        acc[key].jobs += 1;
        acc[key].revenue += Number(row.estimated_cost || 0);
    ...
    filteredRows.forEach((row) => {
      ...
      dayMap[key].jobs += 1;
      dayMap[key].revenue += Number(row.estimated_cost || 0);
    });
    ...
    const profitabilityRows = filteredRows
      .map((row) => ({
        ...
        revenue: Number(row.estimated_cost || 0),
        partsCost: Number(row.parts_cost_total || 0),
        laborCost: Number(row.labor_cost || 0),
        profit: Number(row.job_profitability ?? Number(row.estimated_cost || 0) - Number(row.actual_cost || 0)),
      }))
  ...
  useEffect(() => {
    ...
    onPrepared({
      exportColumns: [
        ...
        { label: "Total", value: (row) => Number(row.estimated_cost || 0) },
