...
export default function ProfitLossReportsContent({
  ...
  const analytics = useMemo(() => {
    ...
    const repairRevenue = filteredRepairs.reduce((acc, row) => acc + Number(row.estimated_cost || 0), 0);
    ...
    filteredRepairs.forEach((repair) => {
      ...
      if (!monthMap[key]) return;
      monthMap[key].repairRevenue += Number(repair.estimated_cost || 0);
    ...
    if (previousYearCompare) {
      ...
      repairRows
        ...
        .forEach((repair) => {
          ...
          if (!previousYearMap[key]) previousYearMap[key] = { revenue: 0 };
          previousYearMap[key].revenue += Number(repair.estimated_cost || 0);
        });
    }
    ...
    const ytdRevenue =
      ...
      ytdRepairs.reduce((acc, row) => acc + Number(row.estimated_cost || 0), 0);
    const ytdCogs =
      ytdSales.reduce(
