...
export default function ExportCenterContent({
  ...
  const reportDataByKey = useMemo(() => {
    ...
    cleanRepairs.forEach((repair) => {
      ...
      if (!key) return;
      if (!dailyMap[key]) dailyMap[key] = { day: key, salesRevenue: 0, repairRevenue: 0, salesCount: 0, repairCount: 0 };
      dailyMap[key].repairRevenue += Number(repair.estimated_cost || 0);
    ...
    cleanRepairs.forEach((repair) => {
      ...
      if (!month) return;
      if (!monthlyMap[month]) monthlyMap[month] = { month, revenue: 0, expenses: 0, tax: 0 };
      monthlyMap[month].revenue += Number(repair.estimated_cost || 0);
    });
    ...
    const outstandingRepairRows = cleanRepairs
      .map((repair) => {
        const total = Number(repair.estimated_cost || 0);
    ...
    cleanRepairs.forEach((repair) => {
      ...
      technicianMap[tech].revenue += Number(repair.estimated_cost || 0);
    });
    ...
    const repairLedgerRows = cleanRepairs.map((repair) => ({
      ...
      estimate: Number(repair.estimated_cost || 0),
      advance: Number(repair.advance_payment || 0),
      balance: Math.max(0, Number(repair.estimated_cost || 0) - Number(repair.advance_payment || 0)),
    }));
      ...
      <div className="xl:col-span-4 space-y-3">
                  ...
                  <Badge
                    tone={
                      String(row.status || "").toLowerCase().includes("fail")
                        ? "red"
                        : String(row.status || "").toLowerCase().includes("queued")
                          ? "amber"
                          : "green"
                    }
                  >
        ...
        <SectionCard title="Legend">
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="rounded-lg border border-white/10 bg-white/5 p-2 flex items-center gap-2"><Mail size={14} /> Email queue</div>
