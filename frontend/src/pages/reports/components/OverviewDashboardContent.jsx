...
export default function OverviewDashboardContent({
  ...
  const analytics = useMemo(() => {
    ...
    completedRepairs.forEach((repair) => {
      ...
      const value = Number(repair.estimated_cost || 0);
    ...
    const repairDueRows = repairRows
      .map((repair) => ({
        ...repair,
        due: Math.max(0, Number(repair.estimated_cost || 0) - Number(repair.advance_payment || 0)),
      }))
    ...
    const totalBilled =
      ...
      completedRepairs.reduce((acc, repair) => acc + Number(repair.estimated_cost || 0), 0);
