...
export default function ActivityLog() {
  ...
  const exportColumns = useMemo(
    () => [
      ...
      { label: "Target Record", value: "target_record" },
      { label: "Old Value", value: (row) => toText(row.old_value) },
      { label: "New Value", value: (row) => toText(row.new_value) },
  ...
  const handleArchive = async (eventId) => {
    ...
    try {
      setArchiveBusy(true);
      const reasonInput = window.prompt("Archive reason (optional):", "") || "";
  ...
  return (
    <div className="h-full min-h-0 overflow-auto custom-scrollbar pr-1 pb-3">
                  ...
                  <tbody>
                    ...
                    {pageRows.map((row) => (
                      ...
                      >
                        ...
                        <td>{row.module || "-"}</td>
                        <td>{row.target_record || "-"}</td>
                        <td title={toText(row.old_value)}>{shortText(row.old_value, 56)}</td>
                        <td title={toText(row.new_value)}>{shortText(row.new_value, 56)}</td>
      ...
      {selectedEvent && (
        <>
          ...
          />
          <aside className="fixed inset-y-0 right-0 z-50 w-full max-w-xl border-l border-white/10 bg-slate-950/95 shadow-2xl">
            <div className="h-full overflow-auto custom-scrollbar p-4">
              ...
              <div className="mt-3 rounded-lg border border-white/10 bg-black/20 p-2.5">
                ...
                <p className="text-slate-200 text-sm mt-1">{selectedEvent.target_record || "-"}</p>
              </div>
