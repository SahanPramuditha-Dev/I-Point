...
const TABS = [
  { id: "store_profile", label: "1. Store Profile", icon: Store },
  { id: "access_control", label: "2. Access Control", icon: Users },
  { id: "business_ops", label: "3. Business Ops", icon: BriefcaseBusiness },
  { id: "financial_settings", label: "4. Financial Settings", icon: Coins },
  { id: "repair_settings", label: "5. Repair Settings", icon: Wrench },
  { id: "invoice_receipt_design", label: "6. Invoice & Receipt Design", icon: Receipt },
  { id: "notifications_alerts", label: "7. Notifications & Alerts", icon: Bell },
  { id: "appearance_display", label: "8. Appearance & Display", icon: Palette },
  { id: "system_apis", label: "9. System & APIs", icon: SettingsIcon },
];
...
function buildLegacyFallbackState({ printProfile, uiPreferences, businessPreferences, integrations, employees, lastBackup }) {
  const state = {
    store_profile: {
      business_identity: {
        shop_name: printProfile?.store_name || "i Store",
...
export default function Settings() {
        ...
        <SectionCard title="Staff List">
          <div className="overflow-x-auto rounded-xl border border-white/10 bg-black/25">
            <Table className="text-xs">
              ...
              <tbody>
                {employees.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-6 text-slate-500">No employees found</td></tr>
                )}
  ...
  if (loading) return <div className="h-full min-h-0 grid place-items-center text-slate-400">Loading settings module...</div>;
    ...
    <div className="flex flex-col h-full gap-4 pb-4">
      ...
      {editingEmployee && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-slate-950 p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-white">Edit Employee</h3>
              <button className="text-slate-400 hover:text-white" onClick={() => setEditingEmployee(null)}>Close</button>
            </div>
